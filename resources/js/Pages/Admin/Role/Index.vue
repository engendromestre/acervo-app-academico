...
import RoleHasPermission from '@/Components/RoleHasPermission.vue';
...
const Submit = () => {
    ...
    if (store.getters.getAction == 'update') {
         setValuesFormField(form);
         form.patch(route("role.update", store.getters.getItem.id), {
             onSuccess: () => {
                 if (form.hasErrors === false) {
                     setTimeout(() => { HideModalState() }, 1000);
                 }
    ...
    if (store.getters.getAction == 'delete') {
        form.delete(route("role.destroy", store.getters.getItem.id), {
            onSuccess: () => {
                if (form.hasErrors === false) {
                    setTimeout(() => { HideModalState() }, 1000);
                }
...
<template>
    ...
    <Head :title="translate('Role')" />
    <AuthenticatedLayout :lang="lang" :breadcrumbs="breadcrumbs">
        <List :path="'/admin/role'" :fields="fields" :data="data" :can="can" :lang="lang" :flash="flash"
            :filters="filters" />
        <Modal :reload="true">
                ...
                <div class="sm:flex sm:items-start">
                    ...
                    <template v-else>
                        <DialogTitle as="h3" class="mt-2 px-2 text-lg font-medium leading-6 text-gray-900 dark:text-white">
                            {{ translate(store.getters.capitalizeAction) }} {{ translate('Role') }}
                        </DialogTitle>
                    </template>
                </div>
                        ...
                        <template v-if="store.getters.getAction == 'update' || store.getters.getAction == 'create'">
                            <CrudLayout>
                                <div class="mt-4" v-for="property, field in fields" :key="field">
                                    <template
                                        v-if="property.dataType == 'text' && (property.update || property.create)">
                                        <div class="mt-2">
                                ...
                                <RoleHasPermission :lang="lang" :permissions="permissions"
                                    :roleHasPermissions="roleHasPermissions" :form="form"></RoleHasPermission>
                            </CrudLayout>
                        </template>
                        <template v-if="store.getters.getAction == 'delete'">
                            <CrudLayout>
