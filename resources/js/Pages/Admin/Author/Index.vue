...
const Submit = () => {
    ...
    if (store.getters.getAction == 'update') {
         setValuesFormField(form);
         form.patch(route("author.update", store.getters.getItem.id), {
             onSuccess: () => {
                 if (form.hasErrors === false) {
                     setTimeout(() => { HideModalState() }, 1000);
                 }
    ...
    if (store.getters.getAction == 'delete') {
        form.delete(route("author.destroy", store.getters.getItem.id), {
            onSuccess: () => {
                if (form.hasErrors === false) {
                    setTimeout(() => { HideModalState() }, 1000);
                }
...
<template>
    ...
    <Head :title="translate('Author')" />
    <AuthenticatedLayout :lang="lang" :breadcrumbs="breadcrumbs">
        <List :path="'/admin/author'" :fields="fields" :data="data" :can="can" :lang="lang" :flash="flash"
            :filters="filters" />
            <Modal>
                ...
                <div class="sm:flex sm:items-start">
                    ...
                    <template v-else>
                        <DialogTitle as="h3" class="mt-2 px-2 text-lg font-medium leading-6 text-gray-900 dark:text-white">
                            {{ translate(store.getters.capitalizeAction) }} {{ translate('Author') }}
                        </DialogTitle>
                    </template>
                </div>
