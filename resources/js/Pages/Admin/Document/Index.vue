...
function formFields(action) {
    ...
    Object.entries(props.fields).forEach((arr) => {
        if (arr[1][action]) {
            jsonFormFields[arr[0]] = arr[1]['value']
        }
    });
...
function setValuesFormField(form) {
    Object.entries(store.getters.getItem).forEach((arr) => {
        if (typeof form[arr[0]] !== 'undefined') {
            if (props.fields[arr[0]].dataType === 'select') {
                form[arr[0]] = arr[1].id;
            } else {
                form[arr[0]] = arr[1];
            }
...
const Submit = () => {
    ...
    if (store.getters.getAction == 'update') {
        setValuesFormField(form);
        form.patch(route("document.update", store.getters.getItem.id), {
            onSuccess: () => {
                if (form.hasErrors === false) {
                    setTimeout(() => { HideModalState() }, 1000);
                }
    ...
    if (store.getters.getAction == 'delete') {
        form.delete(route("document.destroy", store.getters.getItem.id), {
            onSuccess: () => {
                if (form.hasErrors === false) {
                    setTimeout(() => { HideModalState() }, 1000);
                }
...
<template>
    ...
    <AuthenticatedLayout :lang="lang" :breadcrumbs="breadcrumbs">
        ...
        <template v-else>
            <List :path="'/admin/document'" :fields="fields" :data="data" :can="can" :lang="lang" :flash="flash"
                :filters="filters" />
            <Modal>
    ...
    store.getters.getAction == 'delete' ? 'text-red-900' : 'text-blue-900'">
                        ...
                        <template v-else>
                            <DialogTitle as="h3"
                                ...
                                {{ translate(store.getters.capitalizeAction) }} {{ translate('Document') }}
                            </DialogTitle>
                        </template>
                    </div>
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <div class="mt-2">
                            ...
                            <template
                                v-if="(store.getters.getAction == 'update' || store.getters.getAction == 'create')">
                                <CrudLayout>
                                    <div class="mt-4" v-for="property, field in fields" :key="field">
                                        <template v-if="property.update || property.create">
                                            <template v-if="property.dataType == 'select'">
                                                <InputLabel class="" for="field" :value="translate(property.title)" />
                                                <Select :data="fields[field].fixedValues" :field="field"></Select>
                                                <InputError class="mt-2" :message="form.errors[field]" />
                                            </template>
                                            <template v-else-if="property.dataType == 'file'">
                                                <InputLabel class="" for="field" :value="translate(property.title)" />
                                                <Dropzone :lang="lang" :file="fields.file" :field="field">
                                                </Dropzone>
                                                <InputError class="mt-2" :message="form.errors[field]" />
                                            </template>
                                            <template v-else>
                                                <InputLabel class="" for="field" :value="translate(property.title)" />
                                                <TextInput :id="field" :type="property.dataType"
                                                    class="mt-1 block w-full" :autofocus="property.position == 0"
                                                    v-model="store.state.item[field]" />
                                                <InputError class="mt-2" :message="form.errors[field]" />
                                            </template>
                                        </template>
                                    </div>
                ...
                <template v-slot:footer>
                    ...
                    <template v-if="store.getters.getAction == 'update'">
                        <GreenButton :class="{ 'opacity-25': form.processing }" :disabled="form.processing"
                            @click="Submit">
                            {{ translate(store.getters.capitalizeAction) }}
                        </GreenButton>
                    </template>
