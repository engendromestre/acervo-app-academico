...
const model = Object.values(props.modelHasRoles).map((m) => {
    if(rolesId.includes(m.role_id)) {
        const userRole = props.roles.find(obj => obj.id == m.role_id);
        return {'user': m.model_id, 'role': userRole.name }
    }
    
});

Object.values(props.data.data).forEach((d,idx) => {
    model.forEach((m)=> {
        if(m.user == d.id) {
            props.data.data[idx].role = m.role;
        }
    }) ;
});
...
function formFields(action) {
    ...
    Object.entries(props.fields).forEach((arr) => {
        if (arr[1][action]) {
            jsonFormFields[arr[0]] = arr[1]['initValue']
        }
    });
...
const Submit = () => {
    ...
    if (store.getters.getAction == 'update') {
        setValuesFormField(form);
        form.patch(route("user.update", store.getters.getItem.id), {
            onSuccess: () => {
                if (form.hasErrors === false) {
                    setTimeout(() => { HideModalState() }, 1000);
                }
    ...
    if (store.getters.getAction == 'delete') {
        form.delete(route("user.destroy", store.getters.getItem.id), {
            onSuccess: () => {
                if (form.hasErrors === false) {
                    setTimeout(() => { HideModalState() }, 1000);
                }
...
<template>
    ...
    <Head :title="translate('User')" />
    <AuthenticatedLayout :lang="lang" :breadcrumbs="breadcrumbs">
        <List :path="'/admin/user'" :fields="fields" :data="data" :can="can" :lang="lang" :flash="flash"
            :filters="filters" />
        <Modal :reload="true">
                ...
                <div class="sm:flex sm:items-start">
                    ...
                    <template v-else>
                        <DialogTitle as="h3" class="mt-2 px-2 text-lg font-medium leading-6 text-gray-900 dark:text-white">
                            {{ translate(store.getters.capitalizeAction) }} {{ translate('User') }}
                        </DialogTitle>
                    </template>
                </div>
                        ...
                        <template v-if="store.getters.getAction == 'update' || store.getters.getAction == 'create'">
                            <CrudLayout>
                                <div class="mt-4" v-for="property, field in fields" :key="field">
                                    <template
                                        v-if="(property.dataType == 'text' || property.dataType == 'email') && (property.update || property.create)">
                                        <div class="mt-2">
                                ...
                                <RoleUser :lang="lang" :roles="roles" :model="model" :form="form"></RoleUser>
